  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-16">
      <header className="mb-14">
        ...
        <h1 className="mt-3 text-5xl font-bold tracking-tight text-rose-50">
          InboxLift.
        </h1>
        <p className="mt-2 text-3xl text-rose-200/60">
          Gmail AI drafts with a Pro tier that actually has plumbing.
        </p>
        <p className="mt-6 max-w-2xl text-sm leading-relaxed text-rose-200/60">
      ...
      <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {states.map((s) => (
          <Link
            key={s.slug}
            href={`/states/${s.slug}`}
            className="group rounded-2xl border border-white/10 bg-white/[0.03] p-6 transition hover:border-rose-400/40 hover:bg-white/[0.06]"
          >
            <div className="flex items-center justify-between">
              <span className="font-mono text-[11px] tracking-widest text-rose-400/80 uppercase">
                № {s.index} · {s.surface}
              </span>
              <span className="text-rose-200/50 transition group-hover:text-rose-300">
                →
              </span>
            </div>
            <h2 className="mt-4 text-xl font-semibold text-rose-50">
              {s.label}
            </h2>
            <p className="mt-2 text-sm leading-snug text-rose-200/60">
              {s.blurb}
            </p>
          </Link>
        ))}
      ...
      <footer className="mt-20 border-t border-white/10 pt-8 text-sm text-rose-200/50">
        <p>
